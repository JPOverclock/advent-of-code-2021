defmodule Aoc2021.Day4 do
  ...
  def part1(name \\ "day4.txt") do
    {numbers, boards} = input(name)
    play(boards, numbers, [])
  end

  def part2(name \\ "day4.txt") do
    {numbers, boards} = input(name)
  ...
  defp play(boards, [current_number | rest], selected_numbers) do
    ...
    case winning_board do
      [board | _] -> {:ok, {current_number, selected_numbers, board, board_score(board, played_numbers, current_number)}}
      _ -> play(boards, rest, played_numbers)
    end
  end
  ...
  defp play_to_lose(boards, [current_number | rest], selected_numbers) do
    ...
    case remaining_boards do
      [] ->
        [board | _] = boards
        {:ok, {current_number, selected_numbers, board, board_score(board, played_numbers, current_number)}}
      _ -> play_to_lose(remaining_boards, rest, played_numbers)
    end
  end
  ...
  defp winning_board?(board, numbers) do
    any_line? = board
    |> board_lines()
    |> Enum.any?(fn line -> line |> Enum.all?(fn number -> Enum.member?(numbers, number) end) end)
